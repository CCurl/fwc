...
void doNum() { if (state == COMPILE) { lit(pop()); } }
...
void addLit(const char *name, cell val) { addToDict(name); lit(val); comma(EXIT); }
...
void lit(cell n) { if (btwi(n,0,LIT_BITS)) { comma(n|LIT_MASK); } else { comma(LIT); comma(n); } }
